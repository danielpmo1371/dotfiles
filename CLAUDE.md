...
**Wallpaper cycling (`wall-next`)**: `util-scripts/wall-next [next|prev]` cycles the FOCUSED Hyprland monitor's wallpaper via `awww`, in sorted order across every collection under `$WALLPAPER_DIR` (default `~/Pictures/walls`, image collections cloned there directly — not tracked in this repo). Focused monitor comes from `hyprctl monitors -j`; the current image comes from `awww query -j` (awww is the only state, no state file), so an unset or unrecognised current image falls back to the first image (`next`) or last image (`prev`); the list always wraps. `WALL_TRANSITION` (default `grow`) and `WALL_TRANSITION_DURATION` (default `1`) are env-overridable. `hyprland.lua` autostarts `awww-daemon` (replacing hyprpaper; the daemon restores each output's last image from its own cache on restart) and binds `SUPER+SHIFT+N`/`SUPER+SHIFT+B` to it, via the absolute `$HOME/repos/dotfiles/util-scripts/wall-next` path since Hyprland's exec env may lack the shell PATH.
