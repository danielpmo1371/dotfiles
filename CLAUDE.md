...
**Keybinding architecture (single brain)**: `config/tmux/tmux.conf` is the SOLE owner of meta-key (Alt/Option/Cmd) semantics — prefix2 `M-e`, popups, vim-aware pane nav (zero-fork `#{m/r:}` guards on `pane_current_command`, ssh pass-through for meta keys), `M-u`/`M-d` scrollback, `M-r`→`C-r` translation for shell panes only, hazard caps for destructive vi strays (`M-x/M-s/M-c`). Three enforced invariants make this safe: (1) **always inside tmux** — `config/shell/tmux.sh` auto-attaches every interactive local shell (`NO_TMUX=1` escape hatch; SSH/VS Code/no-tty skipped), so no bare vi-mode shell exists to receive unbound ESC+char (which would execute as a vi command); the shells carry NO meta bindings. (2) **popups never host a bare interactive shell** — popup keys bypass all tmux key tables; the `C-s` scratch popup runs a nested tmux session. (3) **parity** — `tests/test-keybinding-parity.sh` asserts every trained meta key has a tmux consumer; adding key #14 means a tmux bind + one list entry. The emulator layer is one line: `config/ghostty/config` sets `key-remap = super=alt` (Ghostty ≥1.3.0, in Brewfile) so Cmd ≡ Option ≡ Alt; menu-conflicting defaults are unbound, Cmd+C/Z stay control bytes via post-remap `alt+c/alt+z` binds (SIGINT/SIGTSTP are kernel tty semantics), Cmd+V keeps native menu paste. On Linux (any emulator, SSH, bare TTY) zero emulator config is needed — Alt works out of the box. tmux `escape-time` is 25ms, never 0 (0 splits ESC+key on slow links). Requires tmux ≥3.2.
...
- `tests/test-keybinding-parity.sh` - Hermetic single-brain invariants: trained meta keys ⊆ tmux consumers, no shell meta bindings, auto-attach guards present
